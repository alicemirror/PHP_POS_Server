<?php
	...
	/*
	 Retrieve the POS products list based on the ID
	 
	 Usage: pos_products?action=get&pos_id=nnn where nnn is the customer pos ID.
	*/
	...
	// Command action(s)
	$cmdGet = 'get_list';		// Retrieves the entire product list
	$cmdGetDept = 'get_dept';	// Retrieves the department product list
	$cmdInsert = 'insert';		// Insert a new product
	...
	if( false == isset($_GET[$ACTION])){ 
		...
		if($command == $cmdGet) {
			...
			$toJson = $manager->getProductsList($pos_id);
